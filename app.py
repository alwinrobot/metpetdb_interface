...
def search_chemistry():
    ...
    #Get samples for (O, M)
        ...
        for s in sample_resources:
            ...
            sample_id = s['sample'].replace("Sample #", "")
            sample_ids.add(sample_id)
        sample_results = api.sample.get(params={'sample_id__in': (',').join(str(s)
                                        for s in sample_ids), 'fields': 'sample_id,user__name,collector,number,public_data,rock_type__rock_type,subsample_count,chem_analyses_count,image_count,minerals__name,collection_date', 'limit':0}).data['objects']
        return json.dumps(sample_results)
...
def subsample(id):
    ...
    filter = {"subsample__subsample_id": subsample['subsample_id'],
              "limit": "0"}
    chemical_analyses = api.chemical_analysis.get(
                                                params=filter).data['objects']
